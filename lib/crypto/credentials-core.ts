// The actual AES-256-GCM logic, with NO "server-only" import — so one-off
// scripts run via plain `tsx` (which never go through Next.js's bundler,
// where the server-only guard would normally be aliased away) can import
// this directly. App code should import from ./credentials.ts instead,
// which re-exports these with the server-only guard on top.
