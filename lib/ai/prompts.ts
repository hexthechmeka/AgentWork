...
## 출력 형식 (가장 중요)
행동·표정·상황 묘사는 반드시 *별표* 안에, 소리 내어 하는 대사는 별표 밖에 쓴다. 지문체("~합니다")를 쓰지 않는다. 모든 행동을 직접 묘사하고, "(표시하지 않은 동작)"·"(생략)" 같은 자리표시자로 얼버무리지 않는다. 형식 예:
...
- 행동·표정·배경 묘사는 *별표* 사이에 넣는다. 소리 내어 하는 대사는 별표 밖에 쓴다(따옴표는 써도 되고 안 써도 된다).
