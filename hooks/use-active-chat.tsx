...
import { getProjectsKey } from "@/components/chat/sidebar-projects";
...
export function ActiveChatProvider({
  ...
  const isProjectView = projectIdOverride !== undefined;
  const chatIdFromUrl = chatIdOverride ?? extractChatId(pathname);
  const isNewChat = !chatIdFromUrl;
  const projectIdFromUrl = projectIdOverride ?? searchParams.get("projectId");
  ...
  const [currentEffort, setCurrentEffort] =
    useState<EffortLevel>(DEFAULT_EFFORT_LEVEL);
  const currentEffortRef = useRef(currentEffort);
  useEffect(() => {
    currentEffortRef.current = currentEffort;
  }, [currentEffort]);

  const projectIdRef = useRef(projectIdFromUrl);
  useEffect(() => {
  ...
  const {
    ...
    onError: (error) => {
      if (error instanceof ChatbotError) {
        ...
        toast({
          description: error.message || "이런, 오류가 발생했어요!",
          type: "error",
        });
      }
    },
